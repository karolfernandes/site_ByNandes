...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['acao'] ?? '') === 'atualizar') {
  try {
    ...
    if ($id <= 0) {
      redirect_with('../PAGINAS_LOjISTA/promocoes_lojista.html', ['erro_banner' => 'ID inválido para edição.']);
    }
    ...
    if ($erros) {
      redirect_with('../PAGINAS_LOjISTA/promocoes_lojista.html', ['erro_banner' => implode(' ', $erros)]);
    }
    ...
    redirect_with('../PAGINAS_LOjISTA/promocoes_lojista.html', ['editar_banner' => 'ok']);

  } catch (Throwable $e) {
    redirect_with('../PAGINAS_LOjISTA/promocoes_lojista.html', ['erro_banner' => 'Erro ao editar: ' . $e->getMessage()]);
  }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['acao'] ?? '') === 'excluir') {
  try {
    $id = (int)($_POST['id'] ?? 0);
    if ($id <= 0) {
      redirect_with('../PAGINAS_LOjISTA/promocoes_lojista.html', ['erro_banner' => 'ID inválido para exclusão.']);
    }
    ...
    redirect_with('../PAGINAS_LOjISTA/promocoes_lojista.html', ['excluir_banner' => 'ok']);

  } catch (Throwable $e) {
    redirect_with('../PAGINAS_LOjISTA/promocoes_lojista.html', ['erro_banner' => 'Erro ao excluir: ' . $e->getMessage()]);
  }
}
...
try {
  if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    redirect_with('../PAGINAS_LOjISTA/promocoes_lojista.html', ['erro_banner' => 'Método inválido']);
  }
  ...
  redirect_with('../PAGINAS_LOjISTA/promocoes_lojista.html', ['cadastro_banner' => 'ok']);

} catch (Throwable $e) {
  redirect_with('../PAGINAS_LOjISTA/promocoes_lojista.html', ['erro_banner' => 'Erro no banco de dados: ' . $e->getMessage()]);
}
